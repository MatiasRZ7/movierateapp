...
export const ColumnDisplay = (props: Props) => {
    ...
    const onSuccess = () => {
        toast.success("Rated successfully", {autoClose: 1500});
    }
    const onError = () => {
    ...
    <Grid columns={3} stackable centered verticalAlign="top" padded="vertically">
        {data.map((displayData: DisplayData) => 
            <Grid.Column key={displayData.id}>
                <Card.Group style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <Link to={`/${displayType === DisplayType.Movies ? "movie" : "tvshow"}/${displayData.id}`}>
                    <Card style={{height: 820}} fluid
                     ...
                     description={displayData.overview.slice(0, 350) + "..."}/>
                      {isRated && 
                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                            <Label style={{ padding: '5px 10px', fontSize: '1em' }} color="green">
                     ...
                     <Form style={{marginTop: 10}}>
                        <Form.Group inline>
                            <Form.Field>
                                <Form.Input type="number" min="0" max="10" step="0.5" onChange={(e) => setRating(Number(e.target.value))}
                                 action={{color: "violet",
                                 labelPosition: "right",
                                 icon: "star",
                                 content: "Rate",
